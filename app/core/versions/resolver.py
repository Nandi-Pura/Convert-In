...
OS_NAMES={Vendor.ASA:"Cisco ASA",Vendor.FORTIGATE:"FortiOS",Vendor.PALO_ALTO:"PAN-OS",Vendor.JUNIPER_SRX:"Junos OS",Vendor.CISCO_IOSXE:"Cisco IOS XE"}
PATTERNS={
    ...
    Vendor.CISCO_IOSXE: re.compile(r"^Cisco IOS XE Software, Version\s+(17\.12\.1)\b",re.M|re.I),
}

def version_family(value:str|None):
    if value in {"7.6.4","23.4R2","17.12.1"}: return value
...
def detect_version(text:str,vendor:Vendor)->VersionDetectionResult:
    ...
    value=match.group(1); family=value if vendor in {Vendor.JUNIPER_SRX,Vendor.CISCO_IOSXE} else version_family(value); return VersionDetectionResult(vendor=vendor,os_name=OS_NAMES[vendor],detected_version=value,detected_family=family,status=VersionDetectionStatus.EXACT,method="documented configuration metadata",confidence=VersionConfidence.HIGH)

def resolve_context(text:str,vendor:Vendor,selected:str|None=None)->VersionContext:
    detected=detect_version(text,vendor); family=selected if vendor in {Vendor.JUNIPER_SRX,Vendor.CISCO_IOSXE} else version_family(selected); override=bool(selected and detected.detected_family and family!=detected.detected_family)
