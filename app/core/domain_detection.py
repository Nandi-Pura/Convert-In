...
def detect_domain(text:str)->DomainDetection:
    value=text.lower()
    scores={ConfigDomain.FIREWALL:sum(x in value for x in ("security policies","security zones","asa version","config firewall policy")),ConfigDomain.ROUTER:sum(x in value for x in ("protocols bgp","router bgp","router ospf","routing-options","ip route ","vrf definition")),ConfigDomain.SWITCH:sum(x in value for x in ("ethernet-switching","switchport","port link-type","vlan ","spanning-tree"))}
