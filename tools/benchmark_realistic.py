...
def measure_fortios(tier):
    text=asa(tier);tracemalloc.start();total=time.perf_counter();marks={};start=time.perf_counter();cfg=parse_config(text,Vendor.ASA);marks["parse_cp0"]=time.perf_counter()-start
    mappings=default_mappings(cfg)
    for i,m in enumerate(mappings.interfaces):m.target_interface=f"port{i+1}";m.target_zone=f"port{i+1}";m.confirmed=True
