...
Current target rendering is bounded: PAN-OS 11.1 for firewall IR; Junos 23.4R2 for router IR. Other registered targets are foundations, not conversion claims. CP2 remains target-specific and evidence-gated. Missing version evidence returns `VERSION_NOT_VERIFIED`; missing mappings or semantic context returns `MANUAL_REVIEW`.
